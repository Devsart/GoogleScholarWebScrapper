# -*- coding: utf-8 -*-
"""
Created on Tue Feb  1 10:33:35 2022
...
def verifica_captcha(driver):
    try:
        ...
        input("Aperte Enter quando concluir o reCaptcha")
        return True
    except Exception:
        return True
...
def job():
    words = input("O que você gostaria de pesquisar?")
    n_artigos = input("Quantos artigos gostaria de coletar?")
    n_artigos = int(n_artigos)
    restricao = input("Gostaria de restringir o ano da pesquisa? S/n")
    string_ano = ''
    if(str.lower(restricao) == "s"):
        ano = input("A partir de qual ano deseja coletar as referências?")
    ...
    while contador <= n_artigos:
        ...
        driver.get('https://scholar.google.com.br/scholar?hl=pt-BR&as_sdt=0%2C5&q='+words+string_ano+page_str)
        sleep(3)
        ...
        page += 10
    df = pd.DataFrame(data, columns = ['Titulo', 'Autor Principal','Ano','Journal','Fonte','Citacoes','Link'])
    now = datetime.now()
    date_time = now.strftime("%d-%m-%Y_%H-%M-%S")
    df.to_excel('referencias'+str(date_time)+'.xlsx')
    print (f"Referencias coletadas com sucesso!")
    sleep(1)
    driver.quit()
